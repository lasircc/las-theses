class GenealogyID ():
    def __init__ (self,genID=''):
        ...
        self._genID = genID
        self._originalDict = {'CRC':'colorectal', 'BRC':'breast', 'MEL':'melanoma', 'LNG':'lung', 'OVC':'ovarian', 'GTR':'gastric', 'PAC':'exocrine pancreatic cancer', 'KDC':'Kidney cancer', 'LYM':'Lymphoma', 'THC':'Thyroid cancer', 'CHC':'CHolangioCarcinoma', 'CRL':'ColoRectal cell Line', 'LNL':'Lung cell Line', 'BRL':'Breast cell Line', 'NEC':'NeuroEndocrine cancer', 'HNC':'Head&Neck carcinoma', 'HCC':'HepatoCellular Carcinoma'}
        ...
        self._vectorSampleDict = {'H':'Human', 'X':'Xenos', 'S':'Spheres', 'A':'adherent lines', 'T':'TILS'}
        ...
        self._archivedMaterial = {'000': 'null', 'VT':'Viable', 'SF':'Snap Frozen', 'RL':'RNAlater', 'PL':'Plasma isolation', 'FF':'Formalin Fixed', 'OF':'OCT frozen', 'SI':'serum isolation', 'CH':'ChinaBlack', 'R':'RNA', 'D':'DNA', 'P':'Protein'}
    ...
    # check genealogyID
    def checkGenID(self):
        return self._genID[self._indexes['last_car'][0]:self._indexes['last_car'][0]+self._indexes['last_car'][1]]
    ...
    # set the case code
    def setCaseCode(self, c):
        if len(c) == 4:
            self._genID = self._genID[:self._indexes['caseCode'][0]] + c + self._genID[self._indexes['caseCode'][0]+self._indexes['caseCode'][1]:]
        return self
    ...
    # set the lineage
    def setLineage(self, l):
        if len(l) == 2:
            self._genID = self._genID[:self._indexes['lineage'][0]] + l + self._genID[self._indexes['lineage'][0]+self._indexes['lineage'][1]:]
        return self
    ...
    # set the archiveMaterial2
    def setArchiveMaterial2(self, am2):
        if len(am2) == 2:
            self._genID = self._genID[:self._indexes['archiveMaterial2'][0]] + am2 + self._genID[self._indexes['archiveMaterial2'][0]+self._indexes['archiveMaterial2'][1]:]
        return self

    # set the aliqExtraction2
    def setAliqExtraction2(self, ae):
        if len(ae) == 2:
            self._genID = self._genID[:self._indexes['aliqExtraction2'][0]] + ae + self._genID[self._indexes['aliqExtraction2'][0]+self._indexes['aliqExtraction2'][1]:]
        return self
    ...
    # set the 2derivationGen
    def set2derivationGen(self, dg):
        if len(dg) == 2:
            self._genID = self._genID[:self._indexes['2derivationGen'][0]] + dg + self._genID[self._indexes['2derivationGen'][0]+self._indexes['2derivationGen'][1]:]
        return self
    ...
    def getPrefix(self):
        return  self.getCase() + self.getTissue() + self.getGeneration() + self.getMouse()

    def getTillVT(self):
        return self.getPrefix() + self.getTissueType()
