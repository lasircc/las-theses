...
def miceLoading (request):
    ...
    if request.method == 'POST': # If the form has been submitted..
        ...
        if "start" in request.POST: # the "start" is hard coded into the rendered form in HTML
            ...
            # Note: this section seems not used at all (probably deprecated and moved client side)
...
def finishListing (request):
    ...
    try:
        for barcode in newMice:
            ...
            m = Mice(id_mouse_strain = strain, id_source = source, barcode=barcode,  id_status= status, available_date = datetime.date.today(), gender = g, birth_date = d, arrival_date = arrival_date, created_by = operator)
            m.save()
            ...
            list_report.append(tableReport([barcode, d, arrival_date, datetime.date.today(), strain, gender, status, source, operator])) 
        transaction.commit()
    except Exception, e:    
