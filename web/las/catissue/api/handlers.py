        ...
        try:
            ...
            for samp in lissample:
                ...
                for nome,dizvalori in dizmis.items():
                    print 'dizvalori[value] is:',dizvalori['value']
                    try:
                    ...
                    if nome.lower().startswith('concentration'):
                        ...
                        lmisura=Measure.objects.filter(idInstrument=instr,name='quality',measureUnit='RIN')
                    if len(lmisura)==0:
                        transaction.rollback()
                        transaction.rollback(using='dbhamilton')
                    ...
                    print 'lmisura[0].id is:',lmisura[0].id
                    print 'dizvalori[value] is:',dizvalori['value']
                    print 'valorefloat is:',valorefloat
                ...
                print 'I am in DerivedUpdateValueMeasureHandler and voleffettivo is: ', voleffettivo
                print 'voltot is: ', voltot
                print 'and vol_taken is: ', vol_taken
...
class EditDerivedValueMeasureHandler(BaseHandler):
    allowed_methods = ('GET','POST')
    @method_decorator(get_functionality_decorator)
    @transaction.commit_manually
    @transaction.commit_manually(using='dbhamilton')
    def create(self, request):
        try:
            ...
            print 'req.body is: ', request.body
            print 'req.post is: ', request.POST
            print 'req.get is: ', request.GET
            # items = request.POST.get('aliquot_edit[]')[0]
            # print 'item is: ', items
            ...
            print 'lisplanrobot is:',lisplanrobot
            ...
            lisplanhamilton=PlanHamilton.objects.filter(id__in=lisplanrobot,processed__isnull=False, operator = name)
            print 'lisplanhamilton is:', lisplanhamilton
            for p in lisplanhamilton:
                lisplanfin.append(p.id)
            print 'lisplanfin is:',lisplanfin
            ...
            print 'lisaliqdersched is:',lisaliqdersched
            listaaliq=json.loads(request.POST['aliquot_edit'])
            print 'listaaliq is:', listaaliq

            for aliquot in listaaliq:
                a_genid = aliquot['e_GenID']
                print 'a_genid is:', a_genid
                a_der_id = aliquot['e_der_id']
                print 'a_der_id is:', a_der_id
                a_barcode = aliquot['e_barcode']
                print 'a_barcode is:', a_barcode
                # a_new_vol = aliquot['e_new_vol']
                # print 'a_new_vol is: ', a_new_vol
                a_new_conc = aliquot['e_new_conc_wrap']
                for measure in a_new_conc:
                    measure_id = measure['id_meas']
                    measure_value = measure['e_new_conc']
                    print 'measure id', measure['id_meas'],' is:', measure['e_new_conc']
                    print '--- query block ---'

                    hc_measure_id=Measure.objects.get(id=measure_id)
                    # hc_measure_id=aux_measure_id
                    # print 'a_new_conc is: ', a_new_conc

                    # hamilton.plan = ( name | timestamp | operator | executed | processed | alerted | labwareid )
                    # hamilton.sample = ( genid  | barcode  | volume | concentration | rank | plan_id )
                    # hamilton.measure = ( name | value   | sample_id | run_id | version )
                    # hamilton.run = ( timestamp| plan_id )
                    # print 'measure_id is: ', measure_id
                    # if measure_id == "22" : 
                    #     meas_name = "ConcentrationUV"
                    # elif measure_id == "23" : 
                    #     meas_name = "ConcentrationFluo"
                    # elif measure_id == "24" : 
                    #     meas_name = "260/280"
                    # elif measure_id == "25" : 
                    #     meas_name = "260/380"
                    # else:
                    #     print "Invalid measure type id " + measure_id

                    # print 'meas_name is: ', meas_name
                    # run_id = RunHamilton.objects.filter(plan_id__in = lisplanfin).values_list('id',flat=True)
                    # run_id = RunHamilton.objects.get(plan_id__in = lisplanfin)
                    # print 'run_id is:', run_id
                    # sample_id = SampleHamilton.objects.filter(genid = a_genid, barcode = a_barcode, plan_id__in = lisplanfin).values('id')[0]['id']
                    # abc = SampleHamilton.objects.get(genid = a_genid, barcode = a_barcode, plan_id__in = lisplanfin)
                    # print 'abc is:', abc
                    # print 'id_sample_id_id is:', abc.id
                    # print 'sample_id is:', sample_id
                    # print 'sample_id[0] is:', sample_id[0]
                    # sample_id=sample_id[0]
                    # curr_value_ham = MeasureHamilton.objects.filter(name = meas_name, sample_id = sample_id, run_id = run_id).values_list('value',flat=True)
                    # if (curr_value_ham):
                    #     curr_value_ham=curr_value_ham[0]
                    #     print 'curr_value_ham is: ', curr_value_ham
                    # else:
                    #     print 'curr_value_ham unset '
                    #     curr_value_ham="unset"
                    print 'measure_value is:', measure_value
                    # if ((curr_value_ham != measure_value)):
                    #     print "HAM measure, they are different"
                    # biobanca.measurementevent=(idMeasure | idQualityEvent | value)
                    # biobanca.qualityevent=(| idQualityProtocol | idQualitySchedule | idAliquot | idAliquotDerivationSchedule | misurationDate | insertionDate | operator  | quantityUsed |)
                    # biobanca.aliquotderivationschedule=( idAliquot | idDerivationSchedule | idDerivedAliquotType | idDerivationProtocol | idKit | derivationExecuted | operator | loadQuantity | volumeOutcome | failed | initialDate | measurementExecuted | aliquotExhausted | deleteTimestamp | deleteOperator | validationTimestamp | notes | idPlanRobot | idPlanDilution |)
                    # mysql> select idAliquotDerivationSchedule from qualityevent where operator like '%cottino%' group by idAliquotDerivationSchedule having count(*)>1;
                    
                    # SampleHamilton.objects.filter(plan_id__in = lisplanfin, genid = a_genid).update(volume = a_new_vol, concentration = a_new_conc)
                    # cane=AliquotDerivationSchedule.objects.filter(idPlanRobot__in = lisplanfin, idAliquot = a_id)
                    # AliquotDerivationSchedule.objects.filter(Q(idPlanRobot__in = lisplanfin, id = a_der_id)&Q(Q(operator = name) | Q(operator = ''))&~Q(idDerivationProtocol = None)&Q(idDerivationProtocol__in = lisprotrobot)&Q(deleteTimestamp = None)).update(volumeOutcome = a_new_vol)
                    # canes = QualityEvent.objects.get(idAliquotDerivationSchedule = a_der_id, operator = name)
                    id_aliquot = Aliquot.objects.get(uniqueGenealogyID = a_genid)
                    print 'id_aliquot is:',id_aliquot.id
                    canes = QualityEvent.objects.get(idAliquotDerivationSchedule = a_der_id, idAliquot = id_aliquot.id, operator = name)
                    print 'canes (q_ev_id) is:', canes
                    curr_value = MeasurementEvent.objects.filter(idMeasure = measure_id, idQualityEvent = canes).values_list('value',flat=True)
                    if (curr_value):
                        curr_value=curr_value[0]
                        print 'curr_value is:', curr_value
                    else:
                        print 'curr_value unset '
                        curr_value="unset"
                    print 'measure_value is:', measure_value
                    # if ((curr_value != measure_value)):
                    #     print "BB measurementevent, they are different"
                    
                    # if re.match("^\d+?\.\d+?$", measure_value) is not None:
                    if re.match("^-?[0-9]\d*(\.\d+)?$", measure_value) is not None:
                        measure_value=round(float(measure_value),3)
                        print "round float is:", round(float(measure_value),3)
                        if (curr_value != measure_value): #check here
                            print "BB measurementevent", curr_value, "is different from", measure_value
                            #if the measurement doesnt exist we must create a new record, not updating a new one
                            if (curr_value == "unset" and measure_value):
                                print 'no previous values of this measure in BB, adding a new one'
                                new_measure_insert = MeasurementEvent(idMeasure=hc_measure_id, idQualityEvent=canes, value=measure_value)
                                new_measure_insert.save()
                            else:
                                print 'measure id: ',measure_id,'idQev: ',canes,'measure_value:',measure_value,'curr_value:',curr_value
                                cane = MeasurementEvent.objects.filter(idMeasure = measure_id, idQualityEvent = canes).values_list('value',flat=True)
                                print 'cane (old) is:', cane
                                MeasurementEvent.objects.filter(idMeasure = measure_id, idQualityEvent = canes).update(value = measure_value)
                                cane = MeasurementEvent.objects.filter(idMeasure = measure_id, idQualityEvent = canes).values_list('value',flat=True)
                                print 'cane (updated) is:', cane

                        # if (curr_value_ham != measure_value):
                        #     if (curr_value_ham == "unset" and measure_value):
                        #         print 'no previous values of this measure in HAM, adding a new one'
                        #         # hamilton.measure = ( name | value   | sample_id | run_id | version )
                        #         new_measure_ham_insert = MeasureHamilton(name = meas_name, value = measure_value, sample_id = abc, run_id = run_id, version = 1)
                        #         new_measure_ham_insert.save()
                        #     else:
                        #         print 'measure id: ',measure_id, 'meas_name: ',meas_name, 'sample_id;', sample_id, 'run_id.',run_id, 'measure_value:',measure_value, 'curr_value:',curr_value
                        #         canos = MeasureHamilton.objects.filter(name = meas_name, sample_id = abc, run_id = run_id).values_list('value',flat=True)
                        #         print 'canos (old) is: ', canos
                        #         MeasureHamilton.objects.filter(name = meas_name, sample_id = abc, run_id = run_id).update(value = measure_value)
                        #         canos = MeasureHamilton.objects.filter(name = meas_name, sample_id = abc, run_id = run_id).values_list('value',flat=True)
                        #         print 'canos (updated) is: ', canos

                    else:   #if user wanna delete it's enough to put - as new value and (if a previous measure exists) the entire row is deleted
                        if (measure_value == "-" and curr_value != "unset"):
                            measure_delete = MeasurementEvent.objects.get(idMeasure=hc_measure_id, idQualityEvent=canes, value=curr_value)
                            measure_delete.delete()
                        # if (measure_value == "-" and curr_value_ham != "unset"):
                        #     measure_ham_delete = MeasureHamilton.objects.get(name = meas_name, sample_id = abc, run_id = run_id, value=curr_value_ham)
                        #     measure_ham_delete.delete()
                        else:
                            print 'not a number or invalid chars'

                    print '--- end of query block ---'
                # print 'no number'
            transaction.commit()
            transaction.commit(using='dbhamilton')
            # return {'dizedit':json.dumps(listaaliq)}
            data = json.dumps({'status':'200'})
            return data
            # return {'data':'success'}
            # return {'message': 'no errors!'}
            # return {'dizpos':json.dumps(dizpos),'lisdersched':json.dumps(lisfinale)}
            # return {'dizpos':json.dumps(dizpos),'lisdersched':json.dumps(lisfinale),'dizmisure':json.dumps(dizmisure),'lisfallite':json.dumps(lisfallite),'num_al':num_aliq,'vol_al':vol,'conc_al':concen,'nomeplan':nomplan,'labwareid':labwareid}
        except Exception, e:
            print 'err',e
            transaction.rollback()
            transaction.rollback(using='dbhamilton')
            return {'error': 'Error in editing data'}    
            ...
            try:
                ...
                for val in lista:
                    ...
                    if len(laliq)==0:
                        ...
                        if esperim.name=='CellLineGeneration' and (vettore=='S' or vettore=='A' or vettore=='O' or vettore=='T'):
                            return {'data':'esperimento'}
                        if esperim.name=='CellLineThawing' and (vettore!='S' and vettore!='A' and vettore!='O' and vettore!='T'):
                            return {'data':'esperimento'}
