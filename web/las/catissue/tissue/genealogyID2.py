class GenealogyID ():
    def __init__ (self,genID=''):
        ...
        self._archivedMaterial = {'000': 'null', 'VT':'Viable', 'SF':'Snap Frozen', 'RL':'RNAlater', 'PL':'Plasma isolation', 'FF':'Formalin Fixed', 'OF':'OCT frozen', 'PS':'Paraffin section', 'OS':'OCT section', 'SI':'serum isolation', 'CH':'ChinaBlack', 'R':'RNA', 'D':'DNA', 'P':'Protein','PX':'PAXtube','FR':'Frozen','FS':'FrozenSediment','LS':'Labeled section'}
        # tuple with start index and lenght of the code
        self._indexes= {'origin':(0,3), 'caseCode':(3,4), 'tissue':(7,2), 'sampleVector':(9,1), 'lineage':(10,2), 'samplePassage':(12,2), 'mouse':(14,3), 'tissueType':(17,3), 'archiveMaterial1': (20,1), 'archiveMaterial2': (20,2), 'aliqExtraction1':(21,2), 'aliqExtraction2':(22,2), '2derivation': (23,1), '2derivationGen':(24,2)}
    ...
    def isMouse(self):
        if self.getSampleVector() == 'X' and self._genID[self._indexes['mouse'][0]+self._indexes['mouse'][1]:] == (26-self._indexes['mouse'][0]-self._indexes['mouse'][1])*'0':
            return True
        else:
            return False
    ...
    def isAliquot(self):
        if self._genID[self._indexes['mouse'][0]+self._indexes['mouse'][1]:] != (26-self._indexes['mouse'][0]-self._indexes['mouse'][1])*'0':
            return True
        else:
            return False
