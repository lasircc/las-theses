...
def ReceiveTransferAliquots(request):
    ...
    print 'Operator is:',operatore
    wg_a = list(get_WG())
    print 'wg_a is:', wg_a
    print 'get_functionality is:', get_functionality()
    
    members=[]
    
    #create a list called "members" attaching each wg_users list (w/o duplicates)

    perm=Permission.objects.get(codename='can_view_BBM_admin')
    users_perm=WG_User.objects.filter(user=operatore,permission=perm) #returns a (series of) object(s) depending on # of wg the user belongs to
    print 'users perm is', users_perm

    for wg_element in wg_a:
        wg_b=WG.objects.filter(name=wg_element)
        lisuser2=User.objects.filter(id__in=WG_User.objects.filter(user__is_active=1,WG=wg_b,permission=perm).values_list("user",flat=True)).order_by('username')
        # lisuser2=WG_User.objects.filter(WG=wg_b,user=operatore,permission=perm).values()
        print 'BBM admin users are:', lisuser2

    if users_perm:
            print 'Operator',name,'has permission tissue.can_view_BBM_admin'
            for wg_element in wg_a:
                users_string=WG.objects.get(name=wg_element).users.all().distinct()
                members = members + sorted(set(users_string) - set(members)) 
                print 'members of',wg_element,'are:', members
            print 'members_tot are:', members
    else:
            print 'Operator',name,'has NO permission tissue.can_view_BBM_admin' 
    ...
    try:    
        ...
        # if operatore.has_perm('tissue.can_view_BBM_admin'):
        if users_perm:
            lista2=Transfer.objects.filter(addressTo__in=members,deleteOperator=None,departureExecuted=1,deliveryExecuted=0)
            print 'lista 2',lista2
            variables = RequestContext(request,{'trasferire':lista2,'ricevuto':True})
            return render_to_response('tissue2/transfer/select_request.html',variables)
        else:
...
# def AdminShowTransferList:
#     wg_a = list(get_WG())
#     # get WG members   
#     # add WG members to the addressTo field -> addressTo in (WG_members)
